...
GameState.prototype.setScore = function(player, col, row) {
  var isWin =
      this.checkRuns(player, col, row, 0, 1) || // vertical,세로증가값/가로증가값
      this.checkRuns(player, col, row, 1, 0) || // horizontal,
      this.checkRuns(player, col, row, 1, 1) || // diagonal "/" . 45도 방향.
      this.checkRuns(player, col, row, 1, -1)   // diagonal "\".  135도 방향.
...
GameState.prototype.checkRuns = function(player, col, row, colStep, rowStep) {
  ...
  for (var step = -3; step <= 3; step++) {//중심점 기준으로 0도,-0도, 45도, -45도, 90도 -90도, 135도 -135도 판단.
    if (this.getPlayerForChipAt(col + step * colStep, row + step * rowStep) === player) {
      ...
      if (runCount === 4) {//완전승리.
        ...
        for(var backstep = step; backstep >= step - 3; backstep--) {
          this.winningChips.push({//이겼던 수를 저장.
...
GameState.prototype.getPlayerForChipAt = function(col, row) {
  ...
  if (this.board[col] !== undefined && this.board[col][row] !== undefined) {
    player = this.board[col][row];//2차원보드에 player번호로 세팅됨.
  }
  return player;
}
...
self.addEventListener('message', function(e) { //.postMessage
...
function makeComputerMove(maxDepth) {
  ...
  for (var depth = 0; depth <= maxDepth; depth++) {
    ...
    var isTopLevel = (depth === maxDepth);//is 5level?
  ...
  self.postMessage({
    ...
    winningChips: winningChips,
    isBoardFull: isBoardFull,
    isWinImminent: isWinImminent, //마지막 승리수
    isLossImminent: isLossImminent //마지막 방어수.
  });
}
...
function think(node, player, recursionsRemaining, isTopLevel) {
  ...
  for (var col = 0; col < TOTAL_COLUMNS; col++) {
    if(isTopLevel) {//최고난이도라면 progress 상태로 변경.
      self.postMessage({
    ...
    var row = node.board[col].length;//각 세로줄이 full이 아닌지 판단.
    if (row < TOTAL_ROWS) {
      ...
      if(!childNode.isWin() && recursionsRemaining > 0) {
        ...
        think(childNode, nextPlayer, recursionsRemaining - 1);//재귀호출.
      }
